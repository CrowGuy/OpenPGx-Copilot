...
2. Raise the conflict as a diff exception (DiffReport -> ExceptionReview) that must be resolved before release.
...
```text
The same genotype may match multiple distinct traits only if the diff exception for the overlap is resolved and the release is approved.
```
...
5. The manifest carries a ReleaseApproval, its ValidationReport passed, and every DiffReport exception is resolved by an ExceptionReview.
6. Records generated by a compiler carry certified-compiler (CertifiedCompiler / CompilationRun) and trusted-source (TrustedSourceProfile) provenance; hand-authored v0.1 bootstrap records are held to the same validation and release approval.
```

A rule or evidence record that lacks certified-compiler or trusted-source provenance, or that belongs to an unresolved diff exception, must not enter an active release. OpenPGx reviews the system, release diffs, and exceptions; it does not require per-rule manual review for every generated record.
...
9. Any included rule lacks certified-compiler or trusted-source provenance and is not a hand-authored v0.1 bootstrap record held to the same validation and release approval.
10. Any diff exception (DiffReport -> ExceptionReview) affecting the release is unresolved, or the manifest lacks a ReleaseApproval or a passing ValidationReport.
```
...
5. It carries trusted-source provenance (TrustedSourceProfile) and certified-compiler provenance (CertifiedCompiler / CompilationRun), or is a hand-authored v0.1 bootstrap record held to the same validation and release approval.
6. Safety validation passes (ValidationReport).
7. All diff exceptions affecting it (DiffReport -> ExceptionReview) are resolved.
8. It is pinned by an approved release (RuntimeReleaseManifest with ReleaseApproval).
9. tests pass.
```

OpenPGx reviews the system, release diffs, and exceptions; it does not require per-rule manual review for every generated record. A rule that lacks compiler/source provenance, or that belongs to an unresolved diff exception, must not enter an active release.
